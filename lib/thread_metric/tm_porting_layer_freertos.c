...
/* Define array to remember the test entry functions */
void (*tm_thread_entry_functions[TM_FREERTOS_MAX_THREADS])(void);

/* Wrapper function to match FreeRTOS task signature */
static void tm_task_wrapper(void *pvParameters) {
  int thread_id = (int)pvParameters;
  if (tm_thread_entry_functions[thread_id]) {
    tm_thread_entry_functions[thread_id]();
  }
}
...
int tm_thread_create(int thread_id, int priority,
  ...
  // Store the entry function
  tm_thread_entry_functions[thread_id] = entry_function;

  status = xTaskCreate(tm_task_wrapper, "Thread-Metric test",
                       configMINIMAL_STACK_SIZE, (void *)thread_id,
...
int tm_queue_send(int queue_id, unsigned long *message_ptr) {
  ...
  status = xQueueSendToBack(tm_queue_array[queue_id], (const void *)message_ptr,
                            (TickType_t)0);
...
int tm_queue_receive(int queue_id, unsigned long *message_ptr) {
  ...
  status = xQueueReceive(tm_queue_array[queue_id], (void *const)message_ptr,
                         (TickType_t)0);
...
/* This function waits the specified semaphore.  If successful, the function
   should return TM_SUCCESS. Otherwise, TM_ERROR should be returned.  */
...
int tm_semaphore_put_from_isr(int semaphore_id) {
  ...
  status =
      xSemaphoreGiveFromISR(tm_semaphore_array[semaphore_id], &yield_required);
...
int tm_memory_pool_create(int pool_id) { return TM_SUCCESS; }
